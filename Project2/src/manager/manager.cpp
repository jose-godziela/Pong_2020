...
namespace Godziela_pong
{
	...
	void check_pause()
	{
		...
		if (!game_start)
		{
			DrawText("Press Space or P to resume", GetScreenWidth() / 3, GetScreenHeight() / 4, GAME_FONT, BLUE);
			DrawText("Press ESCAPE to go back to the menu", GetScreenWidth() / 3.5, GetScreenHeight() / 4 - 30, GAME_FONT, BLUE);
